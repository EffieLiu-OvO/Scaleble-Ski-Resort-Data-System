...
//    private static final String SERVER_URL = "http://localhost:8081/CS6650_Get_Server_war_exploded/skiers";
        ...
        while (attempts < MAX_RETRIES) {
            try {
                ...
                if (response.statusCode() == 201) {
                    System.out.println("[Client] POST success - attempt " + (attempts + 1) + ", status code: " + response.statusCode());
                    return true;
                }

                if (response.statusCode() >= 400) {
                    System.out.println("[Client] POST failed - attempt " + (attempts + 1) + ", status code: " + response.statusCode());
                    attempts++;
                ...
                System.out.println("[Client] Exception during POST attempt " + (attempts + 1));
                attempts++;
                if (attempts < MAX_RETRIES) {
                    try {
                        Thread.sleep(RETRY_DELAY_MS);
